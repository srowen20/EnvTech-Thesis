...
# Authenticate Earth Engine and initialize the project
ee.Authenticate()
...
def extract_samples_ee(point: ee.Geometry,
                       ...
                       radius: int) -> list:
    '''
    Joins Sentinel-2 Surface Reflectance with Sentinel-2 Cloud Probability,
    then samples center-pixel values and neighbourhood arrays for each band,
    along with cloud probability (center + patch array).
    ...
    Parameters:
        ...
        radius (int): Pixel radius for neighbourhood arrays (e.g., 5 → ~11×11 at 10 m).

    Returns:
        list: List of dictionaries (one per image) with sampled values and metadata.
    '''
    # Sentinel-2 SR and S2 cloud probability collections
    sr_col = (
    ...
    # Join SR with its matching cloud-probability image by system:index
    ...
    joined_size = joined.size().getInfo()
    if joined_size == 0:
        ...
        return [{
            ...
            'cloud_patch_array': None
        }]

    image_list = joined.toList(joined.size())
    imgs = [ee.Image(image_list.get(i)) for i in range(joined_size)]

    def sample_img(img) -> dict:
        try:
            # Kernel factory for neighbourhood arrays
            kernel = lambda r: ee.Kernel.square(radius=r, units='pixels')
            full_dict = ee.Dictionary({})

            # Band groups by resolution
            bands_10m = ['B2', 'B3', 'B4', 'B8', 'AOT', 'WVP']
            bands_20m = ['B5', 'B6', 'B7', 'B8A', 'B11', 'B12', 'SCL']
            bands_60m = ['B1', 'B9']
            bands_list = [(bands_10m, 10), (bands_20m, 20), (bands_60m, 60)]

            # Center + patch arrays per band group
            for b_group, scale in bands_list:
                center_sample = img.select(b_group).sample(point, scale=scale).first()
                center_dict = ee.Dictionary({f"{b}_center": center_sample.get(b) for b in b_group})

                array_imgs = [img.select([b]).neighborhoodToArray(kernel(radius)).rename(f"{b}_array") for b in b_group]
                patch_img = ee.Image.cat(array_imgs)
                patch_sample = patch_img.sample(point, scale=scale).first()
                patch_dict = ee.Dictionary({f"{b}_array": patch_sample.get(f"{b}_array") for b in b_group})

                full_dict = full_dict.combine(center_dict, overwrite=True)
                full_dict = full_dict.combine(patch_dict, overwrite=True)

            # Cloud probability (center + patch array) from matched cloud image
            cloud_img = ee.Image(img.get('cloud_img'))
            center_sample = cloud_img.select('probability').sample(point, scale=10).first()
            center_prob = center_sample.get('probability') if center_sample else None

            cloud_patch_img = cloud_img.select('probability').neighborhoodToArray(kernel(radius)).rename('cloud_patch_array')
            patch_sample = cloud_patch_img.sample(point, scale=10).first()
            patch_prob_array = patch_sample.get('cloud_patch_array') if patch_sample else None

            full_dict = full_dict.set('cloud_prob_center', center_prob)
            full_dict = full_dict.set('cloud_patch_array', patch_prob_array)
            ...
            # Convert to Python dict and format date
            ...
            return {
                ...
                'cloud_probability': float(props.get('cloud_prob_center')) if props.get('cloud_prob_center') is not None else None,
                'cloud_patch_array': props.get('cloud_patch_array'),
                **{k: props.get(k) for k in props if k not in ['image_id', 'image_date', 'cloud_prob_center', 'cloud_patch_array']}
            }
    ...
    return [sample_img(img) for img in imgs]
...
def main():
    ...
    Returns:
        None
    '''
    parser = argparse.ArgumentParser(description="Run Sentinel-2 sampling pipeline (bands + cloud probability).")
    parser.add_argument("--df_path", type=str,
                        default="data/modelling/baseline_datasets/processed/for_sentinel2_biggerwindow_leastcloudy_collection/20250802_leastcloudy_imagedateassurveydate.csv",
                        help="Soil CSV path.")
    ...
    parser.add_argument("--start_date_offset", type=int, default=32,
    ...
    parser.add_argument("--save_path", type=str,
                        default="./data/raw/20250802_sentinel_bands_and_cloud_0-32days_radius5.txt",
                        help="Txt file to save the sample outputs to.")
    # parser.add_argument("--save_path", type=str, default="./data/raw/20250731_sentinel_bands_and_cloud_62-365days.txt", help="Txt file to save the sample outputs to.")
    parser.add_argument("--num_workers", type=int, default=8,
                        help="Number of workers for parallel processing.")
    parser.add_argument("--radius", type=int, default=5,
                        help="Pixel radius for neighbourhood arrays (e.g., 5 ~ 11x11 at 10 m).")
